...
El sistema SHALL mostrar, tras una confirmación de entrega exitosa (200), una confirmación visual de éxito con un resumen breve (cliente, distancia, comisión) antes de que el rider vuelva a Inicio. Esta confirmación SHALL permanecer visible aunque la orden salga de la lista de órdenes activas como consecuencia de haber llegado a un estado terminal.

#### Scenario: Confirmación de entrega exitosa
- **WHEN** el backend responde 200 al confirmar la entrega
- **THEN** el sistema muestra la confirmación de pedido completado con el resumen de esa orden
