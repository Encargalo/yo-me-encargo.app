...
At the start, leer siempre el archivo de reglas del agente del proyecto (`CLAUDE.md` para Claude Code, `AGENTS.md` u otro equivalente) si existe — fundamenta cualquier hallazgo o recomendación en las convenciones ya establecidas (stack, estructura, testing, restricciones). Tienen prioridad sobre patrones genéricos.
...
- **Leer siempre las reglas del agente del proyecto** (`CLAUDE.md`/`AGENTS.md` u equivalente) si existe, antes de investigar o recomendar — sus convenciones tienen prioridad sobre patrones genéricos.
