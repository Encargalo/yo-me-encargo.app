...
**Flag especial:** Si los argumentos incluyen `--from-archive`, el skill fue invocado desde `openspec-archive-change`. En ese caso **omitir completamente el paso 5** (gestión de rama y commit) — el archive hará su propio commit que incluirá los cambios de specs. (Nota: la versión actual de `openspec-archive-change` aplica el sync de forma INLINE y ya no invoca este skill — este flag se conserva por compatibilidad si el skill se invoca manualmente con él.)
...
5. **Gestionar rama y commit de los cambios**
   ...
   Si el skill fue invocado con `--from-archive`, omitir este paso completamente.
