...
**Flag especial `--from-archive`:** Si los argumentos incluyen `--from-archive`, este workflow fue invocado inline desde `openspec-archive-change`. En ese caso **omitir completamente el paso 6** (gestión de rama y commit) — el archive hará su propio commit que incluirá los cambios de specs.
...
6. **Gestionar rama y commit de los cambios** *(omitir si se invocó con `--from-archive`)*

   Después de aplicar y validar todos los cambios a las main specs:

   a. `git status --short` — si no hay cambios en `openspec/specs/`, no hay nada que commitear; saltar este paso.

   b. `git branch --show-current`:
      - Si la rama actual es `main`, `master` o `develop` → preguntar con **AskUserQuestion** si crear rama nueva. Opciones: "Sí, crear rama `chore/sync-specs-<change-name>`", "No, quedarse en la rama actual".
      - Si ya está en una rama de feature/change → preguntar solo si hacer commit ahora. Opciones: "Sí, hacer commit ahora", "No, lo haré manualmente".
